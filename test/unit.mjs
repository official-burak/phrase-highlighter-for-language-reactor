...

{
  ...
  assert.equal(css.includes("@media (hover: hover) and (pointer: fine)"), true);
...

{
  ...
  for (const rel of files) {
    ...
    if (rel === "src/hover-hold.ts") {
      ...
      assert.equal(src.includes("HOVER_HOLD_LOCK_MQ"), true, rel);
      assert.equal(src.includes("isHoverHoldLockEnv"), true, rel);
      assert.equal(src.includes("(hover: hover) and (pointer: fine)"), true, rel);
...

{
  ...
  assert.equal(LRPH.isPhraseSaveUrl("https://www.languagereactor.com/api/createSavedItem"), true);
  assert.equal(LRPH.isPhraseSaveUrl("https://www.languagereactor.com/api/base_dict_getHoverDictEntriesForSubs"), false);
  assert.equal(LRPH.isPhraseSaveUrl("https://www.languagereactor.com/api/getSavedItems"), false);
  assert.equal(LRPH.isPhraseSaveUrl("https://www.languagereactor.com/api/getSavedPhrases"), false);
  assert.equal(LRPH.isPhraseSaveUrl("https://www.languagereactor.com/api/listSavedItems"), false);
  assert.equal(LRPH.isPhraseSaveUrl("https://www.languagereactor.com/api/searchSavedItems"), false);
  assert.equal(LRPH.isPhraseSaveUrl("https://www.languagereactor.com/api/exportSavedItems"), false);
  assert.equal(LRPH.isPhraseSaveUrl("https://www.languagereactor.com/saved-items"), false);
  assert.equal(LRPH.isPhraseSaveReadUrl("https://www.languagereactor.com/api/getSavedItems"), true);
  assert.equal(LRPH.isPhraseSaveReadUrl("https://www.languagereactor.com/saved-items"), true);
  assert.equal(LRPH.isPhraseSaveWriteMethod("POST"), true);
  assert.equal(LRPH.isPhraseSaveWriteMethod("GET"), false);
  assert.equal(LRPH.isPhraseSaveWriteMethod(""), true);
  assert.equal(LRPH.isPhraseSaveWriteMethod(undefined), true);

  assert.equal(LRPH.shouldBlockPhraseSaveRequest("https://www.languagereactor.com/api/saveSubtitle", null, "POST"), true);
  assert.equal(LRPH.shouldBlockPhraseSaveRequest("https://www.languagereactor.com/api/saveSubtitle", null, "GET"), false);
  assert.equal(LRPH.shouldBlockPhraseSaveRequest("https://www.languagereactor.com/api/getSavedItems", null, "GET"), false);
  assert.equal(LRPH.shouldBlockPhraseSaveRequest("https://www.languagereactor.com/api/getSavedItems", null, "POST"), false);
  assert.equal(LRPH.shouldBlockPhraseSaveRequest("https://www.languagereactor.com/api/getSavedPhrases", null, "POST"), false);
  assert.equal(LRPH.shouldBlockPhraseSaveRequest("https://www.languagereactor.com/saved-items", null, "GET"), false);
  assert.equal(
  ...
  sandbox.location.href = "https://www.languagereactor.com/saved-items";
  assert.equal(LRPH.isNonWatchRoute(), true);
  assert.equal(LRPH.shouldBlockPhraseSaveRequest("https://www.languagereactor.com/api/saveSubtitle", null, "POST"), false);
  sandbox.location.href = savedHref;
...

{
  ...
  assert.equal(css.includes(`@media ${LRPH.HOVER_HOLD_LOCK_MQ}`), true);
  assert.equal(LRPH.HOVER_HOLD_LOCK_MQ, "(hover: hover) and (pointer: fine)");
...
{
  const prevMq = sandbox.matchMedia;
  ...
  assert.equal(LRPH.isHoverHoldLockEnv(), false);
  const touchCue = mockEl("div", "lln-subs");
  const touchStar = mockEl("button", "MuiIconButton-root");
  touchStar.setAttribute("aria-label", "Save sentence");
  touchCue.appendChild(touchStar);
  assert.equal(LRPH.shouldBlockCuePointer(touchCue), false);
  assert.equal(LRPH.shouldBlockCuePointer(touchStar), false);
  assert.equal(
    LRPH.shouldBlockSaveShortcutKey({ key: "r", ctrlKey: false, metaKey: false, altKey: false, isTrusted: true }),
    false
  );
  ...
  LRPH.disarmHoverHold();
  assert.equal(LRPH.isHoverHoldArmed(), true);
  if (prevMq === undefined) delete sandbox.matchMedia;
  else sandbox.matchMedia = prevMq;
  LRPH.disarmHoverHold();
  assert.equal(LRPH.isHoverHoldArmed(), false);
}
