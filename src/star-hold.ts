...
import { isLrSaveOrStarControl, isNonWatchRoute } from "./selectors";
...
/**
 * Write-only phrase save endpoints. Must not match get/list/search/export or
 * the /saved-items page (those hung when the old pattern aborted them).
 */
const PHRASE_SAVE_WRITE_URL =
  /(?:^|[/?&#_.=/-])(?:save(?:Subtitle|Phrase|Expression|Cue|Sentence|Item|Line|Context)|toggle(?:Saved)?(?:Phrase|Star|Item)|add(?:Saved)?(?:Phrase|Subtitle|Expression|Item)|createSavedItem|star(?:Subtitle|Phrase|Cue)|saveItem|saveContext|subs[_-]?save(?:Cue|Phrase|Subtitle)?)/i;

/** List/get/search/export and the Saved Phrases page itself. */
const PHRASE_SAVE_READ_URL =
  /(?:^|\/)saved-items(?:\/|$|\?|#)|(?:^|[/?&#_.=-])(?:get|list|load|fetch|query|search|find|export|download|count|read|browse)[A-Za-z0-9_-]*(?:saved|save)|(?:^|[/?&#_.=-])(?:saved|save)[-_]?(?:items?|phrases?|words?|cues?)[-_]?(?:list|get|search|export|count)/i;

const WRITE_HTTP = /^(?:POST|PUT|PATCH|DELETE)$/i;
...
export function isPhraseSaveReadUrl(url: unknown): boolean {
  const s = String(url || "");
  if (!s) return false;
  if (/gethoverdict|base_dict_/i.test(s)) return true;
  return PHRASE_SAVE_READ_URL.test(s);
}

export function isPhraseSaveUrl(url: unknown): boolean {
  const s = String(url || "");
  if (!s) return false;
  if (isPhraseSaveReadUrl(s)) return false;
  return PHRASE_SAVE_WRITE_URL.test(s);
}

/** True for POST/PUT/PATCH/DELETE. Empty/missing method (sendBeacon) counts as write. */
export function isPhraseSaveWriteMethod(method?: unknown): boolean {
  if (method == null || method === "") return true;
  return WRITE_HTTP.test(String(method).trim());
}

export function shouldBlockPhraseSaveRequest(url: unknown, body?: unknown, method?: unknown): boolean {
  ...
  if (!isPhraseSaveWriteMethod(method)) return false;
...
export function shouldBlockCuePointer(target: EventTarget | null | undefined, armed?: boolean): boolean {
  try {
    if (!isHoverHoldLockEnv()) return false;
