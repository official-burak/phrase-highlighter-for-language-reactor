 ...
 * Touch / coarse-pointer (no fine hover) skips the lock so LR taps stay live.
 */
...
/** Fine-pointer hover only; touch tablets and phones do not match. */
export const HOVER_HOLD_LOCK_MQ = "(hover: hover) and (pointer: fine)";
...
/** True only when fine-pointer hover lock should run (desktop mouse). */
export function isHoverHoldLockEnv(): boolean {
  try {
    if (typeof matchMedia !== "function") return true;
    return matchMedia(HOVER_HOLD_LOCK_MQ).matches;
  } catch {
    return true;
  }
}
...
/** Fine-pointer only until html.lrph-hover-armed. Descendants need none !important. */
export function hoverHoldCssText(): string {
  ...
  return `@media ${HOVER_HOLD_LOCK_MQ} {
html:not(.${armed}) :is(
  ${hosts}
),
html:not(.${armed}) :is(
