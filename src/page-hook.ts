...
type HookedXHR = XMLHttpRequest & { __lrphUrl?: string | URL; __lrphMethod?: string };

function requestMethod(input: unknown, init?: RequestInit): string {
  if (init && typeof init.method === "string" && init.method) return init.method;
  if (input && typeof input === "object") {
    const rec = input as { method?: unknown };
    if (typeof rec.method === "string" && rec.method) return rec.method;
  }
  return "GET";
}
...
function installHook(): void {
  ...
  if (typeof XMLHttpRequest === "function" && XMLHttpRequest.prototype) {
    ...
    XMLHttpRequest.prototype.send = function (this: HookedXHR) {
      ...
      if (shouldBlockPhraseSaveRequest(openUrl, bodyArg, this.__lrphMethod)) {
        try {
          this.abort();
        } catch {
          /* ignore */
  ...
  if (typeof origFetch === "function") {
    window.fetch = function (this: Window, input: RequestInfo | URL, init?: RequestInit) {
      ...
      if (shouldBlockPhraseSaveRequest(url, bodyArg, method)) {
